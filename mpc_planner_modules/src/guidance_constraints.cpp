...
namespace MPCPlanner
{
    std::vector<CandidateTrajectory> GuidanceConstraints::getCandidateTrajectories() const
    {
        std::vector<CandidateTrajectory> out;
        out.reserve(planners_.size());
        for (size_t i = 0; i < planners_.size(); ++i)
        {
            const auto &planner = planners_[i];
            if (planner.disabled)
                continue;

            CandidateTrajectory c;
            c.success = planner.result.success;
            c.is_original = planner.is_original_planner;
            c.is_best = (static_cast<int>(i) == best_planner_index_);
            c.color = planner.result.color;
            c.guidance_id = planner.result.guidance_ID;
            c.traj = Trajectory(_solver->dt, _solver->N);

            if (c.success)
            {
                for (int k = 1; k < _solver->N; ++k)
                {
                    c.traj.add(planner.local_solver->getOutput(k, "x"),
